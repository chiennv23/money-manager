...
/// be selected. The values are of type `T`, the type parameter of the [FRadio]
...
class FRadio<T> extends StatefulWidget {
  ...
  const FRadio({
    ...
    this.size = FRadioSize.size20,
  })  : assert(autofocus != null),
        assert(toggleable != null),
        super(key: key);

  final FRadioSize size;
  ...
  @override
  _FRadioState<T> createState() => _FRadioState<T>();
}

class _FRadioState<T> extends State<FRadio<T>>
  ...
  @override
  void didUpdateWidget(FRadio<T> oldWidget) {
...
class _RadioPainter extends ToggleablePainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final Paint paint = Paint()
      ...
      ..strokeWidth = 2.0;
    canvas.drawCircle(center, sizeValue.outterRadius, paint);

    // Inner circle
    if (!position.isDismissed) {
      paint.style = PaintingStyle.fill;
      canvas.drawCircle(center, sizeValue.innerRadius * position.value, paint);
    }
  }
}
