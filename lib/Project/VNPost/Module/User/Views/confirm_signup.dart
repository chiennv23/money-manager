...
import 'package:coresystem/Project/VNPost/Module/User/Views/change_phone_email.dart';
...
class ConfirmSignup extends StatefulWidget {
  ...
  const ConfirmSignup(
      {Key key, this.phoneNumber, this.username, this.email, this.type})
      : super(key: key);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Text(
              'Xác thực ${widget.type == 1 ? 'số điện thoại' : 'Email'}',
              style: FTypoSkin.title2.copyWith(color: FColorSkin.title),
            ),
            Container(
              padding: EdgeInsets.only(top: 4, bottom: 2),
              child: Text(
                'Hãy nhập mã xác thực được gửi tới ${widget.type == 1 ? 'số điện thoại' : 'Email'}:',
                style: FTypoSkin.subtitle2
                    .copyWith(color: FColorSkin.secondaryText),
              ),
            ),
            Text(
              '${widget.type == 1 ? widget.phoneNumber : widget.email}'
                  .replaceRange(3, 6, '***'),
            ...
            Container(
              ...
              child: FTextButton(
                ...
                child: Text(
                  'Thay đổi ${widget.type == 1 ? 'số điện thoại' : 'Email'}',
                ...
                onPressed: () {
                  if (widget.type == 1) {
                    CoreRoutes.instance
                        .navigatorPushRoutes(ChangePhoneOrEmail(type: 0));
                  } else {
                    CoreRoutes.instance
                        .navigatorPushRoutes(ChangePhoneOrEmail(type: 1));
                  }
                },
              ),
            ),
            ...
            AnimatedContainer(
              ...
              child: AnimatedSwitcher(
                ...
                child: Text(
                  ...
                  style: FTextStyle.regular14_22.copyWith(color: FColors.red6),
                  ...
                  if (_start != 0)
                    ...
                    FTextButton(
                      ...
                      onPressed: () async {
                        ...
                        final model = await UserDA.resendOtp(widget.username,
                            widget.type == 1 ? 'PHONE' : 'EMAIL');
                        if (model.code == 200) {
                          setState(() {
                            oneClick = true;
                          });
              ...
              child: FFilledButton(
                ...
                onPressed: () async {
                  ...
                  if (countdown == 0 && falseTimes < 5) {
                    ...
                    final model = await UserDA.Verify(
                        widget.username,
                        codeController.text,
                        widget.type == 1 ? 'PHONE' : 'EMAIL');
                    if (model.code == 200) {
                      if (widget.email != null &&
                        ...
                        await CoreRoutes.instance
                            .navigatorPushRoutes(ConfirmSignup(
