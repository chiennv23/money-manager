...
class ForgotPConfirmEmail extends StatefulWidget {
  final String email;

  const ForgotPConfirmEmail({Key key, this.email}) : super(key: key);

  @override
  State<ForgotPConfirmEmail> createState() => _ForgotPConfirmEmailState();
}

class _ForgotPConfirmEmailState extends State<ForgotPConfirmEmail> {
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: Text(
                    'Hãy nhập mã xác thực được gửi tới emaili:',
                    style: FTypoSkin.subtitle2
                        .copyWith(color: FColorSkin.secondaryText),
                  ),
                ),
                Text(
                  '${widget.email}'.replaceRange(3, 6, '***'),
                  style:
                      FTypoSkin.title5.copyWith(color: FColorSkin.primaryText),
                ),
                Container(
  ...
  Future init() async {
    falseTimes =
        SharedPreferencesHelper.instance.getInt(key: 'false_${widget.email}') ??
            0;
    countdown =
        SharedPreferencesHelper.instance.getInt(key: 'time_${widget.email}') ??
            0;
    countFail = SharedPreferencesHelper.instance
            .getInt(key: 'countTimerFail_${widget.email}') ??
        0;
    var _time = SharedPreferencesHelper.instance
            .getInt(key: 'countDownTimer_${widget.email}') ??
        0;
      ...
      if (countCD <= 0) {
        setState(() {
          ...
          SharedPreferencesHelper.instance
              .removeKey(key: 'time_${widget.email}');
          SharedPreferencesHelper.instance
              .removeKey(key: 'false_${widget.email}');
          falseTimes = 0;
          countdown = 0;
          countCD = 0;
        });
  ...
  Future<void> incorrectAuthenticator() async {
    setState(() {
      ...
      SharedPreferencesHelper.instance
          .setInt(key: 'false_${widget.email}', val: falseTimes);
    ...
    if (falseTimes == 5) {
      setState(() {
        ...
        SharedPreferencesHelper.instance
            .setInt(key: 'countTimerFail_${widget.email}', val: countFail);
      });
      ...
      if (countFail >= 1) {
        setState(() {
          ...
          SharedPreferencesHelper.instance.setInt(
              key: 'countDownTimer_${widget.email}', val: countdownTimer);
        });
      }
      var _countdown = WConvert.totalseconds(DateTime.now()) + countdownTimer;
      setState(() {
        SharedPreferencesHelper.instance
            .setInt(key: 'time_${widget.email}', val: _countdown);
      });
    }
  }
  ...
  Future<void> correctAuthenticator() async {
    setState(() {
      isCorrect = true;
      SharedPreferencesHelper.instance.removeKey(key: 'time_${widget.email}');
      SharedPreferencesHelper.instance.removeKey(key: 'false_${widget.email}');
      SharedPreferencesHelper.instance
          .removeKey(key: 'countTimerFail_${widget.email}');
      SharedPreferencesHelper.instance
          .removeKey(key: 'countDownTimer_${widget.email}');
