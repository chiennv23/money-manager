...
import 'package:coresystem/Project/VNPost/Module/Notifications/View/notification_index.dart';
...
class _OrderIndexState extends State<OrderIndex> {
  ...
  void initState() {
    ...
    if (UserService.userInfo.avatar != null) {
      _imagesFile = File(UserService.userInfo.avatar);
    }
    super.initState();
  }
  ...
  final _mainMenu = [
    {
      'title': 'Tiếp nhận yêu cầu gửi hàng',
      'icon': StringIcon.dien_luc,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
    {
      'title': 'Quản lý yêu cầu thu gom',
      'icon': StringIcon.nuoc_sach,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
    {
      'title': 'Quản lý đơn hàng gửi cần xử lý phát',
      'icon': StringIcon.truyen_hinh,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
    {
      'title': 'Hiệu chỉnh đơn hàng',
      'icon': StringIcon.internet,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
    {
      'title': 'Quản lý đơn hàng nhận',
      'icon': StringIcon.vnpt,
      'route': CoreRouteNames.ACCOUNT_INFO,
    },
    {
      'title': 'Quản lý đơn hàng gửi nháp',
      'icon': StringIcon.vay_tieu_dung,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
    {
      'title': 'Thống kê sản lượng',
      'icon': StringIcon.cuoc_dt_tra_sau_dt_co_dinh,
      'route': CoreRouteNames.ACCOUNT_INFO
    },
  ];
  ...
  return Row(
    ...
    children: [
      FFilledButton.icon(
          backgroundColor: FColorSkin.transparent,
          child: FIcon(
            icon: FOutlined.qrcode,
            size: 24,
            color: isDark ? FColorSkin.title : FColorSkin.grey1_background,
          ),
          onPressed: () {
            PopupQRCode(context);
          }),
      FFilledButton.icon(
          ...
          onPressed: () {
            CoreRoutes.instance.navigatorPushRoutes(NotificationIndex());
          }),
    ],
  );
}
